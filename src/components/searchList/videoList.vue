  ...
  <div class="wrap">
    <div class="group_center">
      <div class="center_keyword">
        <span class="keyword_span">视频</span>
      ...
      <div class="center_html">
        <div class="flex-wrap video-wrap" v-if="infoList&&infoList.length>0">
          <div v-for="(item,index) in  infoList" :key="index" class="video-list-one">
            <div class="img-wrap-all" @click="goDetail(item)">
              <img :src="item.videoImageUrl" alt class="img-style" />
              <div class="video-img-wrap">
                <img src="@/assets/image/innovateindex/videoImg.png" class="img-style" alt />
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getInfolist(item, index) {
      this.$http.author
        .post("/party/materialCenter/getMaterialVideoList", {
        ...
        .then(({ data }) => {
          if (data.success === true) {
            this.infoList = data.data.videoList;
            this.tableInfo.totalCount = data.data.videoTotal;
          } else {
            console.log("接口调用出错");
          }
        });
    ...
    goDetail(item) {
       this.$router.push({ name: "videoDetail" ,params: { id:item.id,name:this.tapName }});
    },
  },
};
</script>
