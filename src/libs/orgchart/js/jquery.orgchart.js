...
(function (factory) {
  ...
  var OrgChart = function (elem, opts) {
    ...
    this.defaultOptions = {
      ...
      'parentNodeSymbol': 'el-icon-fa-users',
      'draggable': false,
      'direction': 't2b',
      'pan': false,
      'zoom': false,
  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if ($.type(data) === 'object') {
        ...
        $chart.append('<i class="fa el-icon-fa-circle-o-notch el-icon-fa-spin spinner"></i>');
        $.ajax({
          'url': data,
          'dataType': 'json'
        })
    ...
    showSiblingsEnd: function (event) {
      ...
      if (this.isInAction($node)) {
        this.switchHorizontalArrow($node);
        $node.children('.topEdge').removeClass('el-icon-fa-chevron-circle-up').addClass('el-icon-fa-chevron-circle-down');
      }
    },
    //
    ...
    startLoading: function ($edge) {
      ...
      $edge.parent().append('<i class="fa el-icon-fa-circle-o-notch el-icon-fa-spin spinner"></i>')
    ...
    switchVerticalArrow: function ($arrow) {
      $arrow.toggleClass('el-icon-fa-chevron-circle-up').toggleClass('el-icon-fa-chevron-circle-down');
    },
    //
      ...
      if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' || $node.closest('.nodes').data('siblingsLoaded'))) {
        ...
        if ($prevSib.length) {
          if ($prevSib.is('.hidden')) {
            $node.children('.leftEdge').addClass('el-icon-fa-chevron-left').removeClass('el-icon-fa-chevron-right');
          } else {
            $node.children('.leftEdge').addClass('el-icon-fa-chevron-right').removeClass('el-icon-fa-chevron-left');
          }
        }
        var $nextSib = $node.closest('table').parent().next();
        if ($nextSib.length) {
          if ($nextSib.is('.hidden')) {
            $node.children('.rightEdge').addClass('el-icon-fa-chevron-right').removeClass('el-icon-fa-chevron-left');
          } else {
            $node.children('.rightEdge').addClass('el-icon-fa-chevron-left').removeClass('el-icon-fa-chevron-right');
          }
        }
      } else {
        var $sibs = $node.closest('table').parent().siblings();
        var sibsVisible = $sibs.length ? !$sibs.is('.hidden') : false;
        $node.children('.leftEdge').toggleClass('el-icon-fa-chevron-right', sibsVisible).toggleClass('el-icon-fa-chevron-left', !sibsVisible);
        $node.children('.rightEdge').toggleClass('el-icon-fa-chevron-left', sibsVisible).toggleClass('el-icon-fa-chevron-right', !sibsVisible);
      }
    },
    //
    ...
    nodeEnterLeaveHandler: function (event) {
      ...
      if (event.type === 'mouseenter') {
        if ($topEdge.length) {
          ...
          $topEdge.toggleClass('el-icon-fa-chevron-circle-up', !flag).toggleClass('el-icon-fa-chevron-circle-down', flag);
        }
        if ($bottomEdge.length) {
          flag = this.getNodeState($node, 'children').visible;
          $bottomEdge.toggleClass('el-icon-fa-chevron-circle-down', !flag).toggleClass('el-icon-fa-chevron-circle-up', flag);
        }
        if ($leftEdge.length) {
          this.switchHorizontalArrow($node);
        }
      } else {
        $node.children('.edge').removeClass('el-icon-fa-chevron-circle-up el-icon-fa-chevron-circle-down el-icon-fa-chevron-right el-icon-fa-chevron-left');
      }
    },
    //
    ...
    toggleVNodes: function (event) {
      ...
      $toggleBtn.toggleClass('el-icon-fa-plus-square el-icon-fa-minus-square');
      if ($descendants.eq(0).is('.slide-up')) {
        ...
        $descendants.find('.toggleBtn').removeClass('el-icon-fa-minus-square').addClass('el-icon-fa-plus-square');
      }
    },
    //
    createGhostNode: function (event) {
      ...
      nodeCover.setAttribute('width', 200 * scale);
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv.append('<div class="title"><span class="title-label">' + data[opts.nodeTitle] + '</span><span class="title-num">' + data[opts.nodeContent] + '<span class="person">人</span></span></div>')
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        if ((level + 1) > opts.verticalLevel && Number(flags.substr(2,1))) {
          ...
          $nodeDiv.append('<i class="toggleBtn fa el-icon-fa-' + icon + '-square"></i>');
        }
      } else {
        ...
        if(Number(flags.substr(2,1))) {
          $nodeDiv.append('<i class="edge verticalEdge bottomEdge fa"></i>')
            .children('.title').prepend('<i class="fa '+ data.parentNodeSymbol + ' symbol"></i>');
        }else{
          $nodeDiv.children('.title').prepend('<i class="fa '+ data.parentNodeSymbol + ' symbol"></i>');
        }
      }
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      if (!$mask.length) {
        $chartContainer.append('<div class="mask"><i class="fa el-icon-fa-circle-o-notch el-icon-fa-spin spinner"></i></div>');
      } else {
